...
# <-OpenPose: ros_openposeとの同期を始めます。（<-ESP32オンで送信）
...
UDP_RESV_IP="192.168.11.30" # このPCのIPアドレス(BB70version)"
#UDP_RESV_IP="192.168.11.34" # このPCのIPアドレス(19E0version)
...
flag_OpenPose_action = 0    # OpenPose用の計算モーション送信のオンオフフラグ
...
def meridian_loop():
    ...
    #恐らく r = 右 ????????????????////
    global r_openpose
    global r_openpose_LPF
    
    global l2_openpose
    global l2_openpose_LPF
        ...
        with closing(sock):
            while True:
                ...
                if (checksum[0] == r_meridim[MSG_SIZE-1]) and (frame_sync_r_resv != frame_sync_r_last): 
                    ...
                    #→今回はデモモーションではなくて、echo.pyからパブリッシュされたトピックをここでサブスクライブしている。
                    ...
                    #右肩ロール
                    r_openpose_LPF += 0.1*(-r_openpose_LPF + r_openpose)
                    s_meridim_motion[55] =  round(100 * r_openpose_LPF)
                    """

                    
                    if flag_OpenPose_action:
                        
                        """
                        ここで制御......
                        先生からプログラミングしていただいた、コードをチェックボックスで呼び出す
                        """
                ...
                #    print("pass************")      
                        ...
                        #下のコードはecho.pyからサブスクライブしたものを受け取っている箇所
                        #動いてない
...
def set_OpenPose_action():# チェックボックスに従いアクション送信フラグをオンオフ
    global flag_OpenPose_action
    if flag_OpenPose_action == 0 :
        flag_OpenPose_action = 1
        print("Start OpenPose Sync.")
        print(s_meridim_motion[55])
    else:
        flag_OpenPose_action = 0
        print("Quit OpenPose Sync.")
...
def openpose_cb(Pub_Conv_deg):
    ...
    global r_openpose
    global l2_openpose
    global motor_state_topic

    r_openpose = Pub_Conv_deg.data[0]
    l2_openpose = Pub_Conv_deg.data[1]
    """
    Motor_temp = np.array([s_meridim_motion[25], s_meridim_motion[55]])

    print("Motor hairetu", Motor_temp)

    motor_state_topic = Int16MultiArray(data=Motor_temp)  # numpy arrayをpython listに変換してセット

    print("Motor", motor_state_topic)
    """
...
def main():
    ...
    while(True):
        ...
        with dpg.window(label="Command", width=335, height=190,pos=[260,185]):
            ...
            dpg.add_checkbox(label="<-OpenPose", tag="Action",  callback=set_OpenPose_action, pos=[192,73])
        ...
        while dpg.is_dearpygui_running():
            ...
            # モーターの値のrosbagファイルの生成============================================================================
            if flag_OpenPose_action:
                

                motor_state_topic = rospy.Publisher('motor_state_topic', Int16MultiArray, queue_size=10, latch=True)
                ...
                # Motorの状態を見る部分
                file_path_state = '/home/kajilab/catkin_ws/src/ros_openpose/rosbag_file/rosbag_MotorState'
                file_name_state = f'Pub_score_{date_string}.bag'
                MotorBag = rosbag.Bag(file_path_state + '/' + file_name_state, 'w')
                #print("Bag file saved at:", file_path_state + '/' + file_name_state)
                
                py_time = rospy.Time.from_sec(time.time())

                print("Start",py_time)

                now_rosbag = rospy.Time.now()
                print("now",now_rosbag)
                
                try:
                    MotorBag = rosbag.Bag(file_path_state + '/' + file_name_state, 'w')  # fileに書き込み
                    MotorBag.write('/motor_state_topic', motor_state_topic, now-start)
                finally:
                    if not rospy.is_shutdown():
                        MotorBag.close()
            # =======================================================================================================================
            
            #echo.pyからのOpenPose_thetaのサブスクライブ(受信)
            #ここじゃないと、Subscribe出来ない？？？？？？
            """
            rospy.init_node('ros_openpose', anonymous=True)
            KRS = rospy.Subscriber('ros_openpose_theta',Int16)
            print(KRS)
