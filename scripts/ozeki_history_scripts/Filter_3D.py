...
def kalman_filter(measurements):
    initial_state = 2.0  # 初期状態の推定値
    initial_estimate_error = 0.001  # 初期推定誤差の共分散行列
    ...
    for measurement in measurements:
        ...
        if not np.isnan(measurement):
            ...
            # もしmeasurementがNaNの場合は、予測されたstateを更新されたstateとして使用します
...
# 使用例
measurements = np.array([1.7, np.nan, 7.3, 7.7, 7.9,1.7])  # 観測値のnumpy配列
# カルマンフィルタの実行
